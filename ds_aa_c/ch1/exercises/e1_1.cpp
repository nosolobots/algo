 ...
 * Exercise 1.1
 *
 *  Write a program to solve the selection problem. Let k = N/2. Draw a table
...
void fillArray(int n, int* arr) {
    for (int i = 0; i < n; i++) arr[i] = std::rand() % 10'000;  // [0, 10000)
}
...
int main() {
    ...
    for (int i = 1; i < NTESTS; i++) {
        ...
        std::cout << n << "\t\t" << arr[n / 2] << "\t\t" << elapsed_sec.count()
                  << "s\n";
